...
def test_should_retry_item_only_for_pending_or_in_progress_with_incomplete_sheet():
    assert should_retry_item(status="pendente", sheet_complete=False) is True
    assert should_retry_item(status="processando", sheet_complete=False) is True
    assert should_retry_item(status="sucesso", sheet_complete=False) is False
    assert should_retry_item(status="pendente", sheet_complete=True) is False


def test_reconcile_item_state_uses_sharepoint_for_finalize_and_local_report_for_retry():
    result = reconcile_item_state(
        item_status="processando",
        sheet_complete=True,
